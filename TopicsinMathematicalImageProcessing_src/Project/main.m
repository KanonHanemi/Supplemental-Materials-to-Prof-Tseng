...
% This is the version that only outputs a file that the doctors requested
%
...
image_name = "20190711_084340_002433282J_.jpg";
output_filename_with_format = "20190711_084340_002433282J_.xlsx";
image_output_filename_with_format = "output.jpg";
...
text = cell(35,1);
text_position = NaN(35,2);
box_color = cell(35,1);
shape_name = ["";""];
point = 1;

for freq = 1:10
    y_position_addition_rate = 0;
    %clc;
    ...
    for dB = 1:26
        ...
        for shape_number = 1:size(blue_shape,2)
            ...
            if final_blue_result == 1
                ...
                if shape_number == 1 %x
                    shape_record(point,:) = [point f(freq) 0 decibel(dB) 1 0 0];
                    text_position(point,:) = [x_cor(freq)-15, 1600+0*y_position_addition_rate];
                    box_color{point} = 'blue';
                    shape_name(point) = "X";
                elseif shape_number == 2 %square
                    shape_record(point,:) = [point f(freq) 0 decibel(dB) 1 1 0];
                    text_position(point,:) = [x_cor(freq)-15, 1600+70*y_position_addition_rate];
                    box_color{point} = 'blue';
                    shape_name(point) = "[ ]";
                end

                point = point + 1;
                y_position_addition_rate = y_position_addition_rate + 1;
            end
            if final_blue_result == 2
                blue_detected = 1;

                if shape_number == 1 %x
                    shape_record(point,:) = [point f(freq) 0 decibel(dB) 1 0 1];
                    text_position(point,:) = [x_cor(freq)-25, 1600+70*y_position_addition_rate];
                    box_color{point} = 'blue';
                    shape_name(point) = "X↘︎";
                elseif shape_number == 2 %square
                    shape_record(point,:) = [point f(freq) 0 decibel(dB) 1 1 1];
                    text_position(point,:) = [x_cor(freq)-25, 1600+70*y_position_addition_rate];
                    box_color{point} = 'blue';
                    shape_name(point) = "[ ]↘︎";
                end
        ...
        for shape_number = 1:size(red_shape,2)
            ...
            if final_red_result == 1
                ...
                if shape_number == 1 %circle
                    shape_record(point,:) = [point f(freq) 1 decibel(dB) 1 0 0];
                    text_position(point,:) = [x_cor(freq)-15, 1600+70*y_position_addition_rate];
                    box_color{point} = 'red';
                    shape_name(point) = "O";
                elseif shape_number == 2 %triangle
                    shape_record(point,:) = [point f(freq) 1 decibel(dB) 1 1 0];
                    text_position(point,:) = [x_cor(freq)-20, 1600+70*y_position_addition_rate];
                    box_color{point} = 'red';
                    shape_name(point) = "/\";
            ...
            if final_red_result == 2
                red_detected = 1;
                if shape_number == 1 %circle
                    shape_record(point,:) = [point f(freq) 1 decibel(dB) 1 0 1];
                    text_position(point,:) = [x_cor(freq)-25, 1600+70*y_position_addition_rate];
                    box_color{point} = 'red';
                    shape_name(point) = "O↘︎";
                elseif shape_number == 2 %triangle
                    shape_record(point,:) = [point f(freq) 1 decibel(dB) 1 1 1];
                    text_position(point,:) = [x_cor(freq)-25, 1600+70*y_position_addition_rate];
                    box_color{point} = 'red';
                    shape_name(point) = "/\↘︎";
                end
            ...
            for shape_number = 1:size(blue_shape_half,2)
                ...
                if final_blue_result == 1
                    if shape_number == 1 %large
                        shape_record(point,:) = [point f(freq) 0 decibel(dB) 0 0 0];
                        text_position(point,:) = [x_cor(freq)-15, 1600+70*y_position_addition_rate];
                        box_color{point} = 'blue';
                        shape_name(point) = ">";
                    elseif shape_number == 2 %right square
                        shape_record(point,:) = [point f(freq) 0 decibel(dB) 0 1 0];
                        text_position(point,:) = [x_cor(freq)-15, 1600+70*y_position_addition_rate];
                        box_color{point} = 'blue';
                        shape_name(point) = "]";
                    end
                ...
                if final_blue_result == 2
                    if shape_number == 1 %large
                        shape_record(point,:) = [point f(freq) 0 decibel(dB) 0 0 1];
                        text_position(point,:) = [x_cor(freq)-25, 1600+70*y_position_addition_rate];
                        box_color{point} = 'blue';
                        shape_name(point) = ">↘︎";
                    elseif shape_number == 2 %right square
                        shape_record(point,:) = [point f(freq) 0 decibel(dB) 0 1 1];
                        text_position(point,:) = [x_cor(freq)-25, 1600+70*y_position_addition_rate];
                        box_color{point} = 'blue';
                        shape_name(point) = "]↘︎";
                    end
                ...
                if final_red_result == 1
                    if shape_number == 1 %small
                        shape_record(point,:) = [point f(freq) 1 decibel(dB) 0 0 0];
                        text_position(point,:) = [x_cor(freq)-10, 1600+70*y_position_addition_rate];
                        box_color{point} = 'red';
                        shape_name(point) = "<";
                    elseif shape_number == 2 %left square
                        shape_record(point,:) = [point f(freq) 1 decibel(dB) 0 1 0];
                        text_position(point,:) = [x_cor(freq)-10, 1600+70*y_position_addition_rate];
                        box_color{point} = 'red';
                        shape_name(point) = "[";
                    end
                ...
                if final_red_result == 2
                    if shape_number == 1 %small
                        shape_record(point,:) = [point f(freq) 1 decibel(dB) 0 0 1];
                        text_position(point,:) = [x_cor(freq)-20, 1600+70*y_position_addition_rate];
                        box_color{point} = 'red';
                        shape_name(point) = "<↘︎";
                    elseif shape_number == 2 %left square
                        shape_record(point,:) = [point f(freq) 1 decibel(dB) 0 1 1];
                        text_position(point,:) = [x_cor(freq)-20, 1600+70*y_position_addition_rate];
                        box_color{point} = 'red';
                        shape_name(point) = "[↘︎";
                    end
...
text_position = rmmissing(text_position);
box = cell(size(shape_record,1),1);
for i = 1:size(shape_record,1)
    box{i} = box_color{i};
end
%shape_name = rmmissing(shape_record);
TF = isempty(shape_record);
shape_record = string(shape_record);
shape_record(:,8) = shape_name;
if TF
    fprintf("Detection Failed, try another figure\n");
else
    output = array2table(shape_record,"VariableNames",{'Point', 'Hz', 'Laterality', 'Level', 'Conduction', 'Masked', 'Overwhelm', 'Shape name'});
    writetable(output,output_filename_with_format);
    fprintf("Detection Completed, please check the output file.\n");
end
RGB = insertText(I,text_position,shape_name,FontSize=36,TextBoxColor=box, ...
    BoxOpacity=0.80,TextColor="white");
imwrite(RGB,image_output_filename_with_format);
figure
imshow(RGB)